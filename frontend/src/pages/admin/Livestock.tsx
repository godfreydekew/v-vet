...
import { fetchLivestock, type Species, type HealthStatus } from '@/lib/services/livestock.service';
...
const SPECIES_OPTIONS: (Species | 'all')[] = ['all', 'cattle', 'sheep', 'goat', 'poultry', 'pig', 'other'];
const HEALTH_OPTIONS: (HealthStatus | 'all')[] = ['all', 'healthy', 'sick', 'recovering', 'deceased'];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:px-8 md:py-10 space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={PawPrint} title="No animals found" description="Adjust your filters to find livestock." />
      ) : (
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5 capitalize">
                    {[
                      animal.species,
                      animal.breed,
                      farmMap[animal.farm_id] ? `@ ${farmMap[animal.farm_id]}` : null,
                    ]
                      .filter(Boolean)
                      .join(' · ')}
                  </p>
