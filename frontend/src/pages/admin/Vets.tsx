...
export default function AdminVets() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data.filter((u: User) => u.role === 'vet');
    },
  });
  ...
  const register = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users', 'vet'] });
      toast({ title: 'Vet registered successfully.' });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:px-8 md:py-10 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">Vets</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {isLoading ? 'Loading…' : `${vets.length} registered vet${vets.length !== 1 ? 's' : ''}`}
          </p>
        </div>

        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button type="button" size="sm" className="gap-1.5"><Plus size={16} /> Register Vet</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Register Vet</DialogTitle></DialogHeader>
            <form
      ...
      <div className="relative">
        ...
        <Input placeholder="Search vets…" className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
      </div>

      {isLoading ? (
        <div className="flex justify-center py-16"><Loader2 size={28} className="animate-spin text-muted-foreground" /></div>
      ) : isError ? (
        <p className="text-center text-sm text-destructive py-10">Unable to load vets. Please refresh and try again.</p>
      ) : filtered.length === 0 ? (
        <EmptyState icon={Stethoscope} title="No vets found" description="Register vets to assign cases." actionLabel="Register Vet" onAction={() => setOpen(true)} />
      ) : (
        <div className="space-y-2">
          {filtered.map((v: User) => (
            <div key={v.id} className="bg-card rounded-xl border border-border p-4 hover:shadow-sm transition-shadow duration-150">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-sm text-foreground">{v.full_name ?? '—'}</p>
